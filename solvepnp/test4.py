...
# Convert the list of object_points to a NumPy array
chessboard_3d = np.array(object_points3d, dtype=np.float32)

# Bounding Boxen der erkannten Figuren
...
# Kamera-Matrix (falls verfügbar)
camera_matrix = None  # Hier sollte deine Kamera-Matrix stehen

# Entfernungskoeffizienten (falls verfügbar)
dist_coeffs = None  # Hier sollten deine Entfernungskoeffizienten stehen

# Schachbrett-Eckpunkte in Bildkoordinaten
image_points = []
...
# Um die 3D-Koordinaten der Figuren zu erhalten, verwenden wir solvePnP
retval, rvecs, tvecs = cv2.solvePnP(chessboard_3d, np.array(image_points, dtype=np.float64), camera_matrix, dist_coeffs, flags=cv2.SOLVEPNP_ITERATIVE)

# Die rvecs und tvecs enthalten die Rotation und Translation
# Hier könntest du die Transformation in das Schachbrett-Koordinatensystem nutzen

# Zum Beispiel: Die Transformation der Bounding Boxes in das Schachbrett-Koordinatensystem
for i, box in enumerate(bounding_boxes):
    rvec = rvecs[i]
    tvec = tvecs[i]
    _, rot_matrix = cv2.Rodrigues(rvec)
    inv_rot_matrix = np.linalg.inv(rot_matrix)
    inv_tvec = -np.dot(inv_rot_matrix, tvec)

    # Berechnen der Schachbrettfelder basierend auf den 3D-Koordinaten
    x, y, _ = inv_tvec
    chessboard_col = chr(int(round(x)))  # Spalte
    chessboard_row = str(int(round(y)))  # Zeile

    print(f"{box['name']}: {chessboard_col}{chessboard_row}")
